...
exports._updateGenero = async (req, res) => {
  try {
    ...
    if (id && data._id) {
      await generoModel.findByIdAndUpdate(id, data);
      res.json({
        msg: `El género ${nombre} (${id}) se actualizó correctamente`,
      });
    } else {
      res.json({
...
exports._deleteGenero = async (req, res) => {
  try {
    ...
    await generoModel.findByIdAndUpdate(id, { activo: false });
